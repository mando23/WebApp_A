/******************************* variables *******************/
			...
			function modelo_js(){
				...
				loader.load('JavaScript/brick_quads.json',
				function (geometry){
					//var material_clone = new THREE.MeshFaceMaterial(materials);
					// Material y agregado la textura
					Material_modelo=new THREE.MeshBasicMaterial({color:Blue});
			...
			$('#show_clone').click(function(){
				fn='green chasis';
				ajax_post();
				modelo_js();
			});

			$('#show_body1-1').click(function(){
				sub_sec='BODY_LAYER1';
				part='red-rect1';
				position='POS3';
				ajax_post2();
	  ...
	  function ajax_post2(){
        ...
        // Create some variables we need to send to our PHP file
        var url = "ajaxphp.php";
        var vars = "sub_sec="+sub_sec+"&part="+part+"&position="+position;
        hr.open("POST", url, true);
        // Set content type header information for sending url encoded variables in the request
        hr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        // Access the onreadystatechange event for the XMLHttpRequest object
